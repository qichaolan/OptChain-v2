...
/**
 * Iron Condor Page Wrapper with AI Integration - Chatless Generative UI
 *
...
export function IronCondorPageWithAI({
  ...
  const handleMessage = useCallback(
    (event: MessageEvent) => {
      ...
      console.log('[IronCondor] Received message:', event.data, 'from origin:', event.origin);
      ...
      if (!allowedOrigins.includes(event.origin)) {
        console.log('[IronCondor] Origin not allowed:', event.origin);
        return;
      }
      ...
      if (data?.type === 'IRON_CONDOR_SIMULATION_UPDATE' && data.payload) {
        console.log('[IronCondor] Simulation update received:', data.payload);
  ...
  useEffect(() => {
    console.log('[IronCondor] Setting up message listener');
    window.addEventListener('message', handleMessage);

    return () => {
      console.log('[IronCondor] Removing message listener');
  ...
  return (
    <div className={`relative w-full h-full ${className}`}>
      ...
      <div className="flex h-full">
        {/* Embedded Iron Condor Page - uses flex-1 to fill remaining space */}
