    ...
    return (
      <div className="max-h-[500px] overflow-y-auto px-2 py-2 space-y-2">
        {condors.map((condor) => (
          ...
          >
            {/* Header row: Expiration + Score */}
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className="px-2 py-1 bg-purple-100 text-purple-700 rounded text-xs font-bold">IC</span>
                <span className="font-medium text-gray-900">{condor.expiration}</span>
                <span className="text-sm text-gray-500">{condor.dte}d</span>
              </div>
              <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                ...
                Score: {formatNumber(condor.combined_score, 2)}
              </span>
            </div>
            {/* Strikes visualization */}
            <div className="bg-gray-50 rounded px-2 py-1.5 mb-2 text-xs">
              <div className="flex items-center justify-between text-gray-600">
                <span>Put: {formatCurrency(condor.long_put)}/{formatCurrency(condor.short_put)}</span>
                <span>Call: {formatCurrency(condor.short_call)}/{formatCurrency(condor.long_call)}</span>
              </div>
            </div>
            {/* Credit, ROR, POP row */}
            <div className="flex items-center justify-between text-sm">
              <div className="flex items-center gap-1">
                ...
                <span className="font-semibold text-green-600">{formatCurrency(condor.total_credit)}</span>
              </div>
              <div className="flex items-center gap-1">
                <span className="text-gray-500">ROR:</span>
                <span className="font-medium">{formatPercent(condor.ror)}</span>
              </div>
              <div className="flex items-center gap-1">
                <span className="text-gray-500">POP:</span>
                <span className="font-medium">{formatPercent(condor.combined_pop)}</span>
              </div>
            </div>
            {/* Max Loss row */}
            <div className="mt-1.5 flex items-center justify-between text-xs text-gray-500">
              <span>Max Loss: <span className="text-red-600">{formatCurrency(condor.max_loss)}</span></span>
              <span>Width: ${condor.width.toFixed(0)}</span>
            </div>
          </div>
        ))}
      </div>
          ...
          <div className="max-w-7xl mx-auto px-2 md:px-4 py-3 md:py-6">
            ...
            <div className="bg-white rounded-lg border shadow-sm p-2 md:p-3 mb-3 md:mb-4">
              ...
              {underlyingPrice > 0 && (
                <div className="md:hidden flex items-center justify-between mt-2 pt-2 border-t border-gray-100 text-xs">
                  ...
                  <span className="text-gray-500">{condors.length} found</span>
                </div>
              )}
            </div>
              ...
              <div className="bg-white rounded-lg border shadow-sm overflow-hidden">
                <div className="p-2 md:p-4 border-b flex flex-col md:flex-row md:items-center md:justify-between gap-1">
                  <div className="flex items-center gap-2 md:gap-3">
                    <h2 className="text-sm md:text-lg font-semibold">Iron Condors</h2>
                    {timestamp && (
            ...
            {isLoading && (
              <div className="bg-white rounded-lg border shadow-sm p-6 md:p-12 text-center">
                ...
                <p className="text-sm md:text-base text-gray-600">Screening iron condors...</p>
              </div>
            )}
            ...
            {!isLoading && condors.length === 0 && !error && (
              <div className="bg-white rounded-lg border shadow-sm p-6 md:p-12 text-center">
                ...
                <h3 className="text-base md:text-lg font-semibold text-gray-900 mb-1 md:mb-2">Screen for Iron Condors</h3>
                <p className="text-sm md:text-base text-gray-600">
                  Adjust filters and click "Screen" to find opportunities.
                </p>
              </div>
            )}
        ...
        {currentMetadata && showAiPanel && (
          <div className="md:hidden fixed inset-0 z-[70]">
            ...
            <div className="absolute bottom-0 left-0 right-0 max-h-[85vh] bg-white rounded-t-2xl shadow-2xl flex flex-col animate-slide-up">
              ...
              {/* Header */}
              <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 bg-gradient-to-r from-purple-600 to-violet-600">
                <div className="flex items-center gap-2">
                  <span className="text-xl">🤖</span>
