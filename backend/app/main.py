...
# Get the frontend origin for frame-ancestors
FRONTEND_ORIGIN = os.getenv("FRONTEND_ORIGIN", "http://localhost:8080 https://optchain.app")
...
async def add_security_headers(request: Request, call_next):
    ...
    # Allow embedding in the same origin and configured frontend origins
    # X-Frame-Options is deprecated in favor of CSP frame-ancestors, but kept for legacy browsers
    ...
    if response.headers.get("content-type", "").startswith("text/html"):
        # Build frame-ancestors from frontend origins
        frame_ancestors = f"'self' {FRONTEND_ORIGIN}"
        response.headers["Content-Security-Policy"] = (
            ...
            "img-src 'self' data:; "
            "font-src 'self'; "
            "connect-src 'self' http://localhost:8080 https://optchain.app; "
